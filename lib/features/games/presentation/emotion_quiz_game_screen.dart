...
class _EmotionQuizGameScreenState extends State<EmotionQuizGameScreen> {
  ...
  void _startLevel() {
    ...
    if (_level == 1) {
      _questions = [
        _QuizQ('😊', 'How does this face feel?', ['Happy', 'Sad', 'Angry', 'Scared'], 0),
        _QuizQ('😢', 'How does this face feel?', ['Excited', 'Sad', 'Surprised', 'Happy'], 1),
        _QuizQ('😠', 'How does this face feel?', ['Sleepy', 'Happy', 'Angry', 'Surprised'], 2),
        _QuizQ('😲', 'How does this face feel?', ['Surprised', 'Sad', 'Bored', 'Angry'], 0),
        _QuizQ('😴', 'How does this face feel?', ['Angry', 'Scared', 'Sleepy', 'Excited'], 2),
      ];
    } else {
      // Level 2 adds scenarios
      _questions = [
        _QuizQ('🧸', 'Tommy lost his favorite toy. How does he feel?', ['Happy', 'Sad', 'Angry', 'Sleepy'], 1),
        _QuizQ('🎁', 'Sarah just got a big present! How does she feel?', ['Happy', 'Scared', 'Bored', 'Angry'], 0),
        _QuizQ('🌩️', 'There is a loud thunderstorm. How does Leo feel?', ['Scared', 'Happy', 'Sleepy', 'Surprised'], 0),
        _QuizQ('🏃', 'Alex ran a long race and is very tired. How does he feel?', ['Angry', 'Happy', 'Excited', 'Sleepy'], 3),
      ];
    }
                ...
                children: [
                  Expanded(
                    child: Text(q.question,
                        ...
                        textAlign: TextAlign.center,
                    ).animate(key: ValueKey(q.question)).fadeIn().slideY(begin: 0.2),
                  ),
                  IconButton(
                    icon: Icon(
                      _isPlayingAudio ? Icons.volume_up_rounded : Icons.volume_up_outlined,
                      color: _isPlayingAudio ? AppColors.primary : AppColors.textSecondary,
                      size: 32,
                    ),
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: borderColor, width: isSelected || (isCorrect && _selected != null) ? 2 : 1),
                        ),
                        child: Center(
                          child: Text(
                            e.value, 
                            style: TextStyle(
                              fontSize: 20, 
                              fontWeight: _selected != null && isCorrect ? FontWeight.bold : FontWeight.w600,
                              color: textColor,
                            ),
                          ),
                        ),
                      ),
                    ).animate().fadeIn(delay: Duration(milliseconds: 100 * e.key)).slideY(begin: 0.2);
                  }).toList(),
                ),
              ),
            ],
